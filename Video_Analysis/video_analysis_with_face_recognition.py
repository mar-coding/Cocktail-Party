...
class ScreenCapture:
    ...
    def __init__(self, monitor_number=1, target_fps=30):
        ...
        Args:
            monitor_number: Monitor to capture (1 = primary, 2 = secondary, etc.)
                           Use 0 for all monitors combined
        ...
        # Get monitor info
        if monitor_number == 0:
            # All monitors combined (bounding box of all displays)
            ...
            if monitor_number >= len(self.mss.monitors):
                raise ValueError(f"Monitor {monitor_number} not available. "
                               f"Available monitors: {len(self.mss.monitors) - 1}")
            self.monitor = self.mss.monitors[monitor_number]
        
        self.width = self.monitor['width']
        self.height = self.monitor['height']
        print(f"Screen capture initialized: {self.width}x{self.height} @ {target_fps}fps")
        print(f"Monitor: {monitor_number if monitor_number > 0 else 'All'}")
    
    def read(self):
        """
        Read a frame from the screen.
        
        Returns:
            (ret, frame) where ret is True if frame was captured, frame is BGR image
        """
        try:
            # Capture screen
    ...
    def get(self, prop):
        """Get video property (for compatibility with cv2.VideoCapture)."""
...
class VideoAnalyzer:
    ...
    def __init__(self, video_source=0, similarity_threshold=0.6, screen_monitor=1):
        """
        Initialize video analyzer with face recognition.
        
        Args:
            video_source: Video source:
                         - 0 or integer: Webcam index
                         - "screen" or "screen:N": Screen capture (N = monitor number, default 1)
                         - Path string: Video file path
            similarity_threshold: Threshold for face matching (0.5-0.7 recommended)
            screen_monitor: Monitor number for screen capture (1 = primary, 2 = secondary, 0 = all)
        """
        ...
        if isinstance(video_source, str) and video_source.lower().startswith("screen"):
            if not SCREEN_CAPTURE_AVAILABLE:
                raise ImportError("Screen capture requires mss library. Install with: pip install mss")
            
            self.use_screen_capture = True
            # Parse monitor number if provided (e.g., "screen:2")
            if ":" in video_source:
                try:
                    self.screen_monitor = int(video_source.split(":")[1])
                except ValueError:
                    print(f"Warning: Invalid monitor number in '{video_source}', using default {screen_monitor}")
            
            self.cap = ScreenCapture(monitor_number=self.screen_monitor, target_fps=30)
        else:
            # Regular video capture (webcam or file)
...
def main():
    ...
    # - Integer (0, 1, 2...): Webcam index
    # - String path: Video file path (e.g., "video_4_people.mp4")
    # - "screen": Screen capture from primary monitor
    # - "screen:1": Screen capture from monitor 1 (primary)
    # - "screen:2": Screen capture from monitor 2 (secondary)
    # - "screen:0": Screen capture from all monitors combined
    
    #VIDEO_SOURCE = "video_4_people.mp4"  # Change to "screen" for screen sharing
    VIDEO_SOURCE = "screen"  # Uncomment to use screen sharing
    # VIDEO_SOURCE = "screen:1"  # Uncomment for primary monitor
